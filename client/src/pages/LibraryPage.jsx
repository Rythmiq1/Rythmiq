...
const LibraryPage = () => {
  ...
  const [error, setError] = useState(null);
    ...
    }, []); // Keep the empty array to run only once
    ...
    const fetchSavedPlaylists = async () => {
        try {
          const token = sessionStorage.getItem('token');
          const response = await axios.get(`http://localhost:8080/playlist/user/${userId}/savedPlaylists`, {
            headers: { Authorization: token },
          });
        ...
        <div className="flex flex-wrap space-x-4">
          {playlists.map((playlist) => (
            <div key={playlist._id} className="flex flex-col items-center">
              <img
                src={playlist.image || defaultImg}  alt={playlist.name}
